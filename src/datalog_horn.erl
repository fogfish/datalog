...
unfold({{}, Fun, _, In}) ->
   unfold({Fun(In), Fun, stream:head(In), stream:tail(In)});
   % case Fun(In) of
   %    {pipe, Eg} ->
   %       io:format("[ p ] ~p~n", [stream:list(Eg)]),
   %       {stream:head(Eg), stream:tail(Eg), Fun, InHead, {}}};
   %    {join, Eg} ->
   %       unfold({Eg, Fun, stream:head(In), stream:tail(In)})
   % end;

unfold({Eg, Fun, InHead, In}) ->
   Head = maps:merge(stream:head(Eg), InHead),
   {Head, {stream:tail(Eg), Fun, InHead, In}}.
